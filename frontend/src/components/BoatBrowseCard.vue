            ...
            <p>
              <span class="columns mt-0 mb-1">
                <span class="col-10 ml-3">
                  <strong><router-link :to="{ name:'BoatProfile',params:{id:boat.id} }"  style="color: #2e6b6b; font-size: 20px">{{boat.name}}</router-link> </strong>
                </span>
                <span class="col-2">
          ...
          <div class="columns">
            ...
            <div
              ...
              <div class="buttons ml-5">
                <button v-if="role ==='ROLE_client' && canReserve !== true" class="button is-success" style="background-color: #2e6b6b" v-on:click="$emit('subscribe', boat.id)">Subscribe</button>
...
export default {
  ...
  mounted() {
    ...
    axios.get("https://rental-app-6.herokuapp.com"+"/api/images/getImage/"+this.boat.images[0].path,{responseType:"blob"})
      .then(response => {
